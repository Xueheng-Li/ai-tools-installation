...
print_header() {
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${BOLD}           macOS AI 工具套装一键安装脚本 v1.0                 ${NC}${CYAN}║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
install_vscode() {
    print_section 6 "Visual Studio Code"
    ...
    if is_cask_installed visual-studio-code || is_installed code; then
        skip "已安装"
        return 0
    fi
...
install_cc_switch() {
    print_section 9 "CC-Switch"

    if is_cask_installed cc-switch; then
        skip "已安装"
        return 0
    fi
...
install_uv() {
    print_section 10 "uv (Python 包管理器)"
...
install_data_tools() {
    print_section 11 "数据处理工具 (pandoc, wget, jq, tree, ffmpeg)"
...
install_python_libs() {
    ...
    local libs=("pandas" "numpy" "matplotlib" "scipy" "openpyxl" "xlrd" "jupyter" "markitdown")

    if $DRY_RUN; then
        info "[DRY-RUN] 将安装 Python 库: ${libs[*]}"
        return 0
    fi

    installing "正在安装 Python 库..."
...
install_skills() {
    print_section 14 "Anthropic Skills (手动安装提示)"
...
install_sysu_awesome_cc() {
    print_section 16 "SYSU Awesome CC"
...
install_clash_verge() {
    print_section 17 "Clash Verge Rev (网络代理)"

    local already_installed=false

    if is_cask_installed clash-verge-rev; then
        skip "已安装"
...
export https_proxy=http://127.0.0.1:7890"
        ...
        # 添加到 .bash_profile (macOS bash 登录 shell 默认读取)
        if [[ -f "$HOME/.bash_profile" ]]; then
            if ! grep -q "http_proxy=http://127.0.0.1:7890" "$HOME/.bash_profile"; then
                ...
                skip "~/.bash_profile 已包含代理配置"
            fi
        fi

        # 添加到 .bashrc (如果存在)
        if [[ -f "$HOME/.bashrc" ]]; then
            if ! grep -q "http_proxy=http://127.0.0.1:7890" "$HOME/.bashrc"; then
                echo "$proxy_lines" >> "$HOME/.bashrc"
                info "已添加到 ~/.bashrc"
            else
                skip "~/.bashrc 已包含代理配置"
            fi
...
print_next_steps() {
    echo ""
    echo -e "${BOLD}下一步:${NC}"
    echo "  1. 打开 CC-Switch 应用配置 API 密钥"
    echo "  2. 或运行 'claude login' 登录 Anthropic 账号"
    echo "  3. 运行 'opencode' 或 'claude' 开始使用"
    echo ""
